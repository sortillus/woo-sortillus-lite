...
(async () => {
  ...
  payload.category_state = { status: 'succeeded', total: 2, processed: 2, last_delta_error: 'Category update failed' };
  categories.click();
  await flush();
  assert.equal(elements['woo-sortillus-lite-category-error'].textContent, 'Category update failed', 'Automatic category errors must be visible');
  assert.equal(products.hidden, false, 'Automatic update errors must preserve completed import status');
  payload.category_state.last_delta_error = null;
  categories.click();
  await flush();
  assert.equal(elements['woo-sortillus-lite-category-error'].textContent, '', 'Successful updates must clear the displayed error');
